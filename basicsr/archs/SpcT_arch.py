...
#         short = x
#         x = self.project_in(x)
#         x1, LRbicx2 = self.dwconv(x).chunk(2, dim=1)
#         x = F.gelu(x1) * LRbicx2
...
class Res_Channel_Attn(nn.Module):
    def __init__(self, c_dim, reduction):
        ...
        modules_body.append(Channel_Atteneion(c_dim, reduction))
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask):  # x: B,C,H,W
        ...
        x = x.permute(0, 3, 1, 2)
        return x  # x: B,C,H,W
    # def save_attn_map(self, attn_map):  # attn_map:()
...
class BasicLayer(nn.Module):
    def __init__(self, c_dim, reduction, RC_depth, RS_depth, depth, windows_size, num_heads):
        ...
        self.body_0 = []
        self.body_1 = []
        self.body_0.extend([Res_Channel_Attn(c_dim, reduction) for _ in range(RC_depth)])
        self.body_0.append(Dconv(c_dim, c_dim, 3, padding='same'))
...
class SpcT(nn.Module):
    def __init__(self,  rgb_mean=(0.4488, 0.4371, 0.4040), c_dim=60, reduction=16, Bsc_depth=4, RS_depth=2, RC_depth=4, depth=2,
                 windows_size=8, num_heads=6):
        super(SpcT, self).__init__()
        self.body = []
        self.mean = torch.Tensor(rgb_mean).view(1, 3, 1, 1)
        self.conv_shallow = nn.Conv2d(3, c_dim, kernel_size=(3, 3), stride=(1, 1), padding='same')
        self.body.extend([BasicLayer(c_dim, reduction, RC_depth, RS_depth, depth, windows_size, num_heads) for _ in range(Bsc_depth)])
        self.conv_before_upsample = nn.Sequential(Dconv(c_dim, c_dim, 3, padding='same'))
        self.upsample = nn.Sequential(Pixelshuffle_Block(c_dim, 3, upscale_factor=4, kernel_size=3))
...
if __name__ == '__main__':
    ...
    swin_lap = SpcT(rgb_mean=(0.4488, 0.4371, 0.4040))
